...
export default function ContentAgent() {
  ...
  const features = [
    { title: 'Auto-Repurposing', description: 'Turn one video into 10+ formats automatically—social cuts, GIFs, stories, reels.', icon: '◈' },
    { title: 'Smart Transcription', description: 'AI-powered transcription with speaker detection and timestamping.', icon: '◇' },
    { title: 'Content Library', description: 'Organize, tag, and search all your video assets intelligently.', icon: '◆' },
    { title: 'Caption Generation', description: 'Auto-generate captions in 50+ languages with brand voice.', icon: '◉' },
    { title: 'Thumbnail Creation', description: 'AI selects the best frames and creates platform-optimized thumbnails.', icon: '★' },
    { title: 'Distribution Ready', description: 'Export in all formats, specs, and aspect ratios automatically.', icon: '✦' },
  ]
    ...
    <div style={{ background: '#0a0a0f', minHeight: '100vh', color: '#fff' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 800, opacity: ready ? 1 : 0, transform: ready ? 'translateY(0)' : 'translateY(40px)', transition: 'all 1s cubic-bezier(0.16,1,0.3,1)' }}>
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <button className="btn-primary" style={{ padding: '16px 32px', fontSize: 16, background: BRAND, color: '#000' }}>Start multiplying</button>
          ...
          <div style={{ display: 'flex', gap: 64, marginTop: 80, opacity: ready ? 1 : 0, transition: 'all 0.8s 0.3s' }}>
            {[{ value: '10x', label: 'Content output' }, { value: '90%', label: 'Time saved' }, { value: '50+', label: 'Languages' }].map((m) => (
      ...
      <section ref={featuresRef as React.RefObject<HTMLElement>} style={{ padding: '120px 48px', background: '#06060a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', opacity: featuresVis ? 1 : 0, transform: featuresVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 700, letterSpacing: '-0.03em' }}>Multiply your content</h2>
          </div>
      ...
      <section ref={ctaRef as React.RefObject<HTMLElement>} style={{ padding: '140px 48px', background: '#0a0a0f' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center', opacity: ctaVis ? 1 : 0, transform: ctaVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          <h2 style={{ fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 700, letterSpacing: '-0.03em', marginBottom: 24 }}>Ready to multiply your content?</h2>
          <p style={{ fontSize: 20, color: '#a1a1aa', marginBottom: 48 }}>Stop creating from scratch. Maximize every video you make.</p>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
            <button className="btn-primary" style={{ padding: '16px 36px', fontSize: 16, background: BRAND, color: '#000' }}>Get started free</button>
      ...
      <footer style={{ padding: 48, background: '#06060a', borderTop: '1px solid #27272a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}><span style={{ fontSize: 16, fontWeight: 700 }}>90 Seconds</span><span style={{ color: '#71717a' }}>/</span><span style={{ fontSize: 14, color: BRAND }}>Content Agent</span></div>
