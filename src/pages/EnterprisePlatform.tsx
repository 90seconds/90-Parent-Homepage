...
export default function EnterprisePlatform() {
  ...
  const features = [
    { title: 'Video Asset Management', description: 'Centralize all video content with smart tagging, search, and organization.', icon: '◆' },
    { title: 'Production Workflows', description: 'Manage briefs, approvals, and delivery across global teams.', icon: '◈' },
    { title: 'Analytics & Reporting', description: 'Track performance, spending, and ROI across all video initiatives.', icon: '◉' },
    { title: 'Team Collaboration', description: 'Comments, reviews, and approvals in one place.', icon: '★' },
    { title: 'Brand Governance', description: 'Ensure consistency with templates, guidelines, and approval flows.', icon: '✦' },
    { title: 'Integrations', description: 'Connect with your existing marketing and DAM systems.', icon: '◇' },
  ]
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 800, opacity: ready ? 1 : 0, transform: ready ? 'translateY(0)' : 'translateY(40px)', transition: 'all 1s cubic-bezier(0.16,1,0.3,1)' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: `${BRAND}15`, padding: '6px 14px', borderRadius: 20, marginBottom: 24 }}>
              <span style={{ fontSize: 12, color: BRAND, fontWeight: 600, letterSpacing: '0.05em' }}>VIDEO OPERATIONS HUB</span>
            </div>
            <h1 style={{ fontSize: 'clamp(48px, 6vw, 80px)', fontWeight: 700, lineHeight: 1, letterSpacing: '-0.04em', marginBottom: 24 }}>Enterprise<br />Platform</h1>
            <p style={{ fontSize: 'clamp(20px, 2.5vw, 28px)', color: BRAND, fontWeight: 500, marginBottom: 24 }}>The system of record for enterprise video.</p>
            <p style={{ fontSize: 18, color: '#a1a1aa', lineHeight: 1.7, maxWidth: 600, marginBottom: 40 }}>Manage global video operations at scale. From asset management to production workflows, analytics to governance—everything in one platform.</p>
            <div style={{ display: 'flex', gap: 16 }}>
              <button className="btn-primary" style={{ padding: '16px 32px', fontSize: 16, background: BRAND }}>Request demo</button>
              <button className="btn-secondary" style={{ padding: '16px 32px', fontSize: 16 }}>Learn more</button>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 64, marginTop: 80, opacity: ready ? 1 : 0, transition: 'all 0.8s cubic-bezier(0.16,1,0.3,1) 0.3s' }}>
            {[{ value: '4,500+', label: 'Enterprise brands' }, { value: '10M+', label: 'Videos managed' }, { value: '99.9%', label: 'Uptime SLA' }].map((m) => (
      ...
      <section ref={featuresRef as React.RefObject<HTMLElement>} style={{ padding: '120px 48px', background: '#06060a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', opacity: featuresVis ? 1 : 0, transform: featuresVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 700, letterSpacing: '-0.03em' }}>Everything you need</h2>
          </div>
      ...
      <section ref={ctaRef as React.RefObject<HTMLElement>} style={{ padding: '140px 48px', background: '#0a0a0f' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center', opacity: ctaVis ? 1 : 0, transform: ctaVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
            <button className="btn-primary" style={{ padding: '16px 36px', fontSize: 16, background: BRAND }}>Request demo</button>
