...
export default function CustomerStory() {
  ...
  const features = [
    { title: 'Self-Service Capture', description: 'Customers record testimonials from their phones with guided prompts.', icon: '★' },
    { title: 'Remote Interviews', description: 'High-quality remote video interviews with your happiest customers.', icon: '◇' },
    { title: 'Story Templates', description: 'Pre-built templates for case studies, testimonials, and success stories.', icon: '◆' },
    { title: 'Auto-Editing', description: 'AI cuts and assembles footage into polished stories automatically.', icon: '◈' },
    { title: 'Quote Extraction', description: 'Pull the best quotes for social, sales decks, and marketing.', icon: '◉' },
    { title: 'CRM Integration', description: 'Connect with Salesforce, HubSpot, and your customer success tools.', icon: '✦' },
  ]
    ...
    <div style={{ background: '#0a0a0f', minHeight: '100vh', color: '#fff' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 800, opacity: ready ? 1 : 0, transform: ready ? 'translateY(0)' : 'translateY(40px)', transition: 'all 1s cubic-bezier(0.16,1,0.3,1)' }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: `${BRAND}15`, padding: '6px 14px', borderRadius: 20, marginBottom: 24 }}>
              <span style={{ fontSize: 12, color: BRAND, fontWeight: 600, letterSpacing: '0.05em' }}>THE PROOF ENGINE</span>
            </div>
            <h1 style={{ fontSize: 'clamp(48px, 6vw, 80px)', fontWeight: 700, lineHeight: 1, letterSpacing: '-0.04em', marginBottom: 24 }}>Customer Story<br />Platform</h1>
            <p style={{ fontSize: 'clamp(20px, 2.5vw, 28px)', color: BRAND, fontWeight: 500, marginBottom: 24 }}>Turn customers into proof.</p>
            <p style={{ fontSize: 18, color: '#a1a1aa', lineHeight: 1.7, maxWidth: 600, marginBottom: 40 }}>Productized B2B customer proof at scale. Capture testimonials, create case studies, and build a library of social proof that converts—all in one platform.</p>
            <div style={{ display: 'flex', gap: 16 }}>
              <button className="btn-primary" style={{ padding: '16px 32px', fontSize: 16, background: BRAND }}>Start capturing</button>
              <button className="btn-secondary" style={{ padding: '16px 32px', fontSize: 16 }}>See examples</button>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 64, marginTop: 80, opacity: ready ? 1 : 0, transition: 'all 0.8s 0.3s' }}>
            {[{ value: '200+', label: 'Stories/quarter' }, { value: '3x', label: 'More testimonials' }, { value: '80%', label: 'Faster production' }].map((m) => (
      ...
      <section ref={featuresRef as React.RefObject<HTMLElement>} style={{ padding: '120px 48px', background: '#06060a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', opacity: featuresVis ? 1 : 0, transform: featuresVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: 'clamp(32px, 4vw, 48px)', fontWeight: 700, letterSpacing: '-0.03em' }}>Build your proof library</h2>
          </div>
      ...
      <section ref={ctaRef as React.RefObject<HTMLElement>} style={{ padding: '140px 48px', background: '#0a0a0f' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center', opacity: ctaVis ? 1 : 0, transform: ctaVis ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s' }}>
          <h2 style={{ fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 700, letterSpacing: '-0.03em', marginBottom: 24 }}>Ready to capture customer proof?</h2>
          <p style={{ fontSize: 20, color: '#a1a1aa', marginBottom: 48 }}>Turn your happiest customers into your best marketing.</p>
      ...
      <footer style={{ padding: 48, background: '#06060a', borderTop: '1px solid #27272a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}><span style={{ fontSize: 16, fontWeight: 700 }}>90 Seconds</span><span style={{ color: '#71717a' }}>/</span><span style={{ fontSize: 14, color: BRAND }}>Customer Story Platform</span></div>
